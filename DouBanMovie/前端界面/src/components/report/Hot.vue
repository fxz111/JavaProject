...
export default {
  ...
  computed: {
    ...
    cateName() {
      ...
      return this.allData[this.activeName].data.list[this.currentIndex].name
    },
    themeStyle() {
  ...
  methods: {
    ...
    // 发送请求，获取数据
    getData() {
      axios.get('http://localhost:8001/MovieService/movie/getActDre').then(response=>{
    ...
    updateChart() {
      // 处理数据
      const legenDateArr = this.allData[this.activeName].data.list[this.currentIndex].children.map(item => item.name)
      const seriesDataArr = this.allData[this.activeName].data.list[this.currentIndex].children.map(item => {
        return {
          name: item.name,
          value: item.value,
    ...
    toLeft() {
      this.currentIndex--
      // 已到达最左边
      if (this.currentIndex < 0) this.currentIndex = this.allData[this.activeName].data.list.length - 1
      this.updateChart()
    },
    // 点击右侧按钮
    toRight() {
      this.currentIndex++
      // 已到达最右边
      if (this.currentIndex > this.allData[this.activeName].data.list.length - 1) this.currentIndex = 0
      this.updateChart()
    },
    // 当前选中的类型
